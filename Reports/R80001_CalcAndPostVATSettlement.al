report 80001 "OP Calc. and Post VAT Stlmt."
{
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/CalcandPostVATSettlement.rdl';
    AdditionalSearchTerms = 'settle vat value added tax,report vat value added tax';
    ApplicationArea = Basic, Suite;
    CaptionML = CSY = 'Vypočítat a účtovat vyrovnání DPH - OP', ENU = 'Calculate and Post VAT Settlement';
    ...
    dataset
    {
        dataitem("VAT Posting Setup"; "VAT Posting Setup")
        {
            ...
            //OP PEKU>>
            column(RoundingAmt; RoundingAmount)
            {

            }
            column(TotalAmt; VATAmount - RoundingAmount)
            {
            ...
            begin
                ...
                IF RoundingAmount <> 0 THEN BEGIN
                    ...
                    RoundingGLAccount.GET(RoundingAccount);

                    GenJnlLine.VALIDATE("Account No.", RoundingGLAccount."No.");
                    ...
                    GenJnlLine."Posting Date" := PostingDate;
                    GenJnlLine."VAT Date" := PostingDate;
                    GenJnlLine."Document Type" := 0;
                    GenJnlLine."Document No." := DocNo;
                    GenJnlLine.Description := 'Zaokrouhlení DPH';
                    GenJnlLine.Amount := RoundingAmount;
                    GenJnlLine."Source Currency Code" := GLSetup."Additional Reporting Currency";
                    IF VATAmount <> 0 THEN
                        GenJnlLine."Source Currency Amount" := ROUND(RoundingAmount * VATAmountAddCurr / VATAmount, 0.01);
                    GenJnlLine."Source Code" := SourceCodeSetup."VAT Settlement";
                    GenJnlLine."VAT Posting" := GenJnlLine."VAT Posting"::"Manual VAT Entry";
                    IF PostSettlement THEN
                        PostGenJnlLine(GenJnlLine);
                    IF VATAmount <> 0 THEN
                        VATAmountAddCurr -= ROUND(RoundingAmount * VATAmountAddCurr / VATAmount, 0.01);
                    VATAmount -= RoundingAmount;
                END;
                //OP PEKU<<
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    //OP PEKU>>
                    field(RoundingGLAccount; RoundingAccount)
                    {
                        ApplicationArea = Basic, Suite;
                        CaptionML = ENU = 'Rounding Account', CSY = 'Účet zaokrouhlení';
                        TableRelation = "G/L Account";
                        ToolTip = 'Specifies the number of the VAT settlement account. Select the field to see the chart of account. This field must be filled in.';
                    }
                    field(RoundingAmount; RoundingAmount)
                    {
                        ApplicationArea = Basic, Suite;
                        CaptionML = ENU = 'Rounding Amount', CSY = 'Částka zaokrouhlení';
                        ToolTip = 'Specifies the number of the VAT settlement account. Select the field to see the chart of account. This field must be filled in.';
                    }
                    //OP PEKU<<
    ...
    var
        ...
        //OP PEKU>>
        RoundingAmtLbl: Label 'Rounding Amount';
        //OP PEKU<<
