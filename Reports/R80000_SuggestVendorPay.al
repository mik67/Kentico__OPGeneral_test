report 80000 "Suggest Vendor Payments OP"
{
    ...
    procedure GatherReleasedInvoices()
    var
        PurchaseHeader: Record "Purchase Header";
        PurchaseHeaderMod: Record "Purchase Header";
    begin
        //On Point get all released invoices
        PurchaseHeader.SetRange("Document Type", PurchaseHeader."Document Type"::Invoice);
        PurchaseHeader.SetRange(Status, PurchaseHeader.Status::Released);
        PurchaseHeader.SetRange("On Hold", '');
        if PurchaseHeader.FindSet() then
            repeat
                PurchaseHeader.CalcFields("Amount Including VAT");
                if PurchaseHeader."Amount Including VAT" <> 0 then begin
                    InsertGenJnlLineFromPH(PurchaseHeader);
                end;
                PurchaseHeaderMod := PurchaseHeader;
                PurchaseHeaderMod."On Hold" := 'UN';
                PurchaseHeaderMod.Modify();
            until PurchaseHeader.Next() = 0;
    end;

    procedure InsertGenJnlLineFromPH(PurchHeader: Record "Purchase Header")
    begin
        with GenJnlLine do begin
            Init;
            Window2.Update(1, PurchHeader."Buy-from Vendor No.");
            LastLineNo := LastLineNo + 10000;
            "Line No." := LastLineNo;
            "Document Type" := "Document Type"::Payment;
            "Posting No. Series" := GenJnlBatch."Posting No. Series";
            ...
            "Account Type" := "Account Type"::Vendor;
            SetHideValidation(true);
            Validate("Account No.", PurchHeader."Buy-from Vendor No.");
